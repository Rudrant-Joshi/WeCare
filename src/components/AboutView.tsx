  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-24 text-left" id="about-view">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <ScrollAnimate variant="fadeLeft" delay={0.2}>
          <motion.div 
            whileHover={{ scale: 1.01 }}
            className="bg-gradient-to-tr from-teal-500 to-indigo-600 p-8 rounded-3xl text-white shadow-xl relative overflow-hidden min-h-[380px] flex flex-col justify-between magnetic-hover"
          >
            <div className="absolute -right-12 -top-12 w-48 h-48 bg-white/10 rounded-full blur-2xl animate-float" />
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8" staggerDelay={0.12}>
          {coreValues.map((value, idx) => (
            <StaggerItem key={idx}>
              <motion.div 
                whileHover={{ y: -6 }}
                className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm card-hover flex flex-col items-start text-left h-full group"
              >
                <div className="p-3 bg-slate-50 rounded-xl mb-4 icon-hover-rotate">
                  {value.icon}
                </div>
                <h3 className="text-slate-900 font-bold text-base mb-2 group-hover:text-teal-600 transition-colors">{value.title}</h3>
      ...
      <section className="bg-slate-50 p-8 sm:p-12 rounded-3xl border border-slate-100 space-y-12">
        ...
        <div className="relative border-l-2 border-teal-200 ml-4 md:ml-32 space-y-12 py-4">
          {milestones.map((mile, idx) => (
            <ScrollAnimate key={idx} variant="fadeRight" delay={0.1 * idx}>
              <div className="relative pl-8 md:pl-16">
                {/* Timeline marker */}
                <div className="absolute -left-2 top-1 w-4.5 h-4.5 rounded-full bg-teal-500 border-4 border-white shadow-md timeline-dot" />
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center bg-white">
            ...
            <div className="grid grid-cols-2 gap-6 pt-4">
              {statistics.map((stat, idx) => (
                <motion.div 
                  ...
                  className="bg-slate-50 p-4 rounded-xl border border-slate-100 stat-hover cursor-default"
                >
        ...
        <ScrollAnimate variant="fadeLeft" delay={0.15}>
          <div className="space-y-4">
            <motion.div 
              whileHover={{ y: -3, scale: 1.01 }}
              className="bg-teal-50/50 p-6 rounded-2xl border border-teal-100 flex items-start gap-4 card-hover-sm"
            >
              <div className="p-3 bg-teal-500 text-white rounded-xl icon-hover-bounce">
            ...
            <motion.div 
              whileHover={{ y: -3, scale: 1.01 }}
              className="bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100 flex items-start gap-4 card-hover-sm"
            >
              <div className="p-3 bg-indigo-500 text-white rounded-xl icon-hover-bounce">
